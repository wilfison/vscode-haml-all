...
### US-001: Higiene de `getAssetDirectories`, `looksLikeData` e dedup com `linter_name` (M9, M6) ✅ (`1ddeb64`)
...
### US-004: Rotas sem TTL, com gatilho por palavra e controllers de engines (M5) ✅ (`3ed3236`)
...
- [x] `extractPartialNameFromLine` também reconhece: `render @user` / `render user` / `render(@user)` → `_user` (identificador sem aspas; `@` removido) e `render @users` → `_users`. **Desvio decidido na implementação:** a singularização **não** acontece aqui, e sim em `resolvePartial`, como *fallback* quando o nome como escrito não casa nada. Motivo: `extractPartialNameFromLine` passa a devolver o que está escrito (honesto e testável), um `_users.html.haml` de verdade continua resolvendo, e a heurística ingênua (`@status` → `statu`) só custa uma busca falha em vez de estragar o nome. O inflector (`ies`→`y`, `ses`/`xes`/`zes`/`ches`/`shes`→ remove `es`, senão remove `s` final) está em `partialIndex.ts`, marcado com `// ponytail: naive inflector`. `collection: @rows` com `partial:` continua usando o `partial:`.
...
- FR-22: `render @x`, `render x`, `render @xs` e `collection: @xs` (sem `partial:`) devem inferir o nome do partial a partir do identificador; quando o nome como escrito não casa nada, uma singularização ingênua é tentada.
...
Decisão do mantenedor pelo escopo completo (`collection:`, `render @user`, variantes, múltiplos `app/views`). O inflector é ingênuo por escolha: a busca por `baseName` resolve `render @user` sem pluralizar, e a singularização só entra como fallback, para `render @users`/`collection:`. Fica em `resolvePartial` (ver US-008), não na extração do nome. Marcado com `ponytail:`.
