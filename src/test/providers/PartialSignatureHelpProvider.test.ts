...
function createViews(locals: string): { viewPath: string; cleanup: () => void } {
  ...
  const partialPath = path.join(viewsDir, '_form.html.haml');
  fs.writeFileSync(partialPath, `-# locals: (${locals})\n`);
  ...
  setPartialIndex([partialFileFrom(partialPath) as PartialFile]);
...
async function activeParameterFor(locals: string, lineContent: string): Promise<number> {
  ...
  try {
    ...
    const help = await provider.provideSignatureHelp(document, position, null as any);
...
suite('PartialSignatureHelpProvider Tests', () => {
  test('keeps the first parameter active with no comma typed', async () => {
    assert.strictEqual(await activeParameterFor('a:, b:, c:', "= render 'form'"), 0);
  });

  test('advances one parameter per comma', async () => {
    assert.strictEqual(await activeParameterFor('a:, b:, c:', "= render 'form', a: 1"), 1);
    assert.strictEqual(await activeParameterFor('a:, b:, c:', "= render 'form', a: 1, b: 2"), 2);
    assert.strictEqual(await activeParameterFor('a:, b:, c:', "= render 'form', a: 1, b: 2, c: 3"), 3);
  });

  test('never goes past the last parameter of the signature', async () => {
    // 3 locals -> location + 3 parameters, so 3 is the highest valid index.
    assert.strictEqual(await activeParameterFor('a:, b:, c:', "= render 'form', a: 1, b: 2, c: 3, d: 4, e: 5"), 3);
    assert.strictEqual(await activeParameterFor('a:', "= render 'form', a: 1, b: 2, c: 3"), 1);
  });
});
